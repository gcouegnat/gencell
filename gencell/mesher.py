...
def create_mesh_from_image(img, smooth_iter=1000, tolerance=1.3, max_area=None):
    ...
    for contour in contours:
        ...
        if pts.shape[0] > 4:
            ...
            if pts.shape[0] > 2:
                if closed:
                    edges.extend(
                        connect_round_trip(len(vertices), len(vertices) + len(pts) - 2)
                    )
    ...
    mesh = triangle(
        vertices, edges
    )  # , quality=True) #, min_angle=30.0, max_area=max_area)

    print("*** Assigning materials")
    ids = []
    for i in range(len(mesh.cells)):
        xc = 0.333333333 * (
            mesh.vertices[mesh.cells[i][0]][0]
            + mesh.vertices[mesh.cells[i][1]][0]
            + mesh.vertices[mesh.cells[i][2]][0]
        )
        yc = 0.333333333 * (
            mesh.vertices[mesh.cells[i][0]][1]
            + mesh.vertices[mesh.cells[i][1]][1]
            + mesh.vertices[mesh.cells[i][2]][1]
        )
